...
<button class="dropbtn" class="fas fa-bars" style="background-color:black">MANAGE USERS
  ...
  padding: 10px;" >
            ...
            <form action="viewcon.php" method="post">
    ...
    width:100px;">
                ...
                <table border="1px">
                    <thead>
                <tr>
                <th colspan="6" style="text-align: center;">Registered Constituency</th>
                </tr>  
                </thead>
                <thead>         
                    <tr>
...
$flag=1;
                          ...
                          if ((!empty($search)))
    {
        if($search==$row["user_name"])
        {
          ...
          echo"<tr><td>".$row["ID"]."</td><td>".$row["user_name"]."</td><td>".$row["region"]."</td><td>"
          .$row["created_at"]."</td>"
        ?> <td> <a href="updatecon.php? conuid=<?php echo $row['ID'] ?>"><button style="width:40px; height:35px; font-size:13px;">
          Update  </button></a> </td><td> <a href="nebe_handler/condelete.php?condid=<?php echo $row['ID'] ?>"><button  
